...
async function migrateMovies() {
  ...
  for (let page = 0; page < numberOfPages; page++) {
    ...
    for (let i = 0; i < movies.length; i++) {
      ...
      let neoMovie = await API_HELPERS.fetchMovie('neo', movie.id);
      ...
      if(neoMovie == undefined) {
        ...
        await API_HELPERS.insertModel('neo', 'movies', movie);
        ...
        for (let key in actors) {
          ...
          if (actors_inserted_ids.indexOf(actor.id) > -1) {
            ...
            await API_HELPERS.insertModel('neo', 'actors', actor);
            actors_inserted_ids.push(actor.id);
          }

          let res = await API_HELPERS.insertRole('neo', actor.id, movie.id, actor.acted_in.character);
        }
